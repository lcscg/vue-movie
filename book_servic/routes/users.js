...
router.post('/findPassword', function(req, res, next) {
    ...
    //这里需要两个返回情况，一个是req.body.repassword存在时，一个是repassword不存在时
    //这个接口同时用于密码的重置，需要用户登录
    if (req.body.repassword) {
        //当存在时需要验证其登录情况或者验证其code
        if (req.body.token) {
            //当存在code登录状态时，验证其状态
            if (!req.body.user_id) {
            ...
            user.findUserPassword(req.body.username, req.body.userMail, req.body.userPhone, function(err, userFound) {
                if (userFound.length != 0) {
                    ...
                    res.json({ status: 1, message: "信息错误1" })
                }
            })
        }
    } else {
        ...
        user.findUserPassword(req.body.username, req.body.userMail, req.body.userPhone, function(err, userFound) {
            if (userFound.length != 0) {
                ...
                res.json({ status: 1, message: "信息错误2" })
            }
        })
    }
});
...
router.post('/sendEmail', function(req, res, next) {
    //验证完整性，这里使用if,可以使用正则表达式进行验证
    if (!req.body.token) {
        res.json({ status: 1, message: "用户登录状态错误1" })
    }
    if (!req.body.user_id) {
        res.json({ status: 1, message: "用户登录状态错误2" })
    }
    ...
    if (req.body.token == getMD5Password(req.body.user_id)) {
        ...
        res.json({ status: 1, message: "用户登录错误3" })
    }
});
